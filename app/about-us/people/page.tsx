              ...
              {filteredMembers.length > 0 ? (
                <div className="flex flex-wrap justify-center gap-6 xl:gap-8">
                  {filteredMembers.map((member, index) => (
                    <div
                      key={index}
                      className={`w-full sm:w-[calc(50%-12px)] md:w-[calc(33.333%-16px)] lg:w-[calc(25%-18px)] xl:w-[calc(20%-25.6px)] rounded-lg border shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col ${mounted && resolvedTheme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
                    >
                      <div className="relative">
                        <div className="w-full h-full flex justify-center items-center">
                          <div className="w-32 h-32 rounded-full overflow-hidden mx-auto border-4 border-gray-300 my-6">
                            <Image
                              src={member.image}
                              alt={member.name}
                              width={128}
                      ...
                      <div className="p-6 text-center flex flex-col h-full">
                        ...
                        <p className={`font-semibold mb-3 ${mounted && resolvedTheme === 'dark' ? 'text-cyan-400' : 'text-cyan-500'}`}>
                          {member.role}
                        </p>
                        
                        <p className={`text-sm leading-relaxed mb-4 flex-grow ${mounted && resolvedTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                          {member.description}
                        </p>
                        
                        <div className="flex justify-center space-x-3 mt-auto">
                          {/* LinkedIn Icon - Only show if link exists */}
                          {member.socialLinks.linkedin && (
                            <a 
                              href={member.socialLinks.linkedin} 
                              className={`${mounted && resolvedTheme === 'dark' ? 'text-gray-400 hover:text-cyan-400' : 'text-gray-500 hover:text-cyan-500'}`}
                            >
                          ...
                          {/* Instagram Icon - Only show if link exists */}
                          ...
                          {/* Email Icon - Only show if link exists */}
