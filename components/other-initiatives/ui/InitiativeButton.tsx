...
import type { InitiativeCta } from "@/types/other-initiatives-page";
...
export function InitiativeButton({
  ...
  const variants: Record<InitiativeCta["variant"], string> = {
  ...
  return (
    <Link href={cta.href} className={linkClass} style={style}>
      {content}
    </Link>
  );
}
