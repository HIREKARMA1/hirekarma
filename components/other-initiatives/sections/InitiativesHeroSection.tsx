...
import {
  getInitiativeSlideAlt,
  getOtherInitiativesMediaSync,
  resolveInitiativeMediaSrc,
} from "@/services/other-initiatives-page";
import { AutoSlideGallery } from "@/components/products-page/ui/AutoSlideGallery";
import { ContainerScroll } from "@/components/products-page/container-scroll/ContainerScroll";
...
export function InitiativesHeroSection() {
  ...
  const { hero } = content;
  const media = getOtherInitiativesMediaSync();

  const slides = media.hero.tabletSlides.map((slide) => ({
    src: resolveInitiativeMediaSrc(slide.src),
    alt: getInitiativeSlideAlt(slide, locale),
  }));
